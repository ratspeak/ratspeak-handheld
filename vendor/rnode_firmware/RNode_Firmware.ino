...
char sbuf[128];
  ...
  // firmware (src/hal/Power.cpp). All peripheral rails up, SD_DET kept input.
  #define TP_XL9555_ADDR      0x20
  #define TP_XL_REG_OUTPUT_0  0x02
  #define TP_XL_REG_CONFIG_0  0x06

  bool tp_xl_write8(uint8_t reg, uint8_t value) {
    Wire.beginTransmission(TP_XL9555_ADDR);
    Wire.write(reg);
    Wire.write(value);
    return Wire.endTransmission() == 0;
  }

  bool tp_xl_read8(uint8_t reg, uint8_t &value) {
    Wire.beginTransmission(TP_XL9555_ADDR);
    Wire.write(reg);
    if (Wire.endTransmission() != 0) return false;
    if (Wire.requestFrom((uint8_t)TP_XL9555_ADDR, (uint8_t)1) != 1) return false;
    value = Wire.read();
    return true;
  }

  bool tp_xl_write16(uint8_t reg_lo, uint16_t value) {
    return tp_xl_write8(reg_lo, value & 0xFF) && tp_xl_write8(reg_lo + 1, value >> 8);
  }

  bool tp_xl_read16(uint8_t reg_lo, uint16_t &value) {
    uint8_t lo = 0, hi = 0;
    if (!tp_xl_read8(reg_lo, lo) || !tp_xl_read8(reg_lo + 1, hi)) return false;
    value = (uint16_t)lo | ((uint16_t)hi << 8);
    return true;
  }

  void tpager_enable_peripheral_rails() {
    uint16_t outputs = 0;
    uint16_t config = 0xFFFF;
    tp_xl_read16(TP_XL_REG_OUTPUT_0, outputs);
    tp_xl_read16(TP_XL_REG_CONFIG_0, config);
    ...
    config |= (1U << 10);      // SD_DET remains input

    tp_xl_write16(TP_XL_REG_OUTPUT_0, outputs);
    tp_xl_write16(TP_XL_REG_CONFIG_0, config);
    delay(20);
  }
#endif
...
void setup() {
    ...
    #elif BOARD_MODEL == BOARD_TPAGER
      // The SX1262, display, keyboard and SD rails are gated by the XL9555
      // expander — bring them up over I2C before any modem/display access.
  ...
  #elif BOARD_MODEL == BOARD_TPAGER
    rs_pager::return_to_launcher_next_boot();
  #endif
  ...
  #if BOARD_MODEL != BOARD_RAK4631 && BOARD_MODEL != BOARD_HELTEC_T114 && BOARD_MODEL != BOARD_TECHO && BOARD_MODEL != BOARD_T3S3 && BOARD_MODEL != BOARD_TBEAM_S_V1 && BOARD_MODEL != BOARD_HELTEC32_V4 && BOARD_MODEL != BOARD_CARDPUTER_ADV && BOARD_MODEL != BOARD_TDECK && BOARD_MODEL != BOARD_TPAGER
  ...
  LoRa->setPins(pin_cs, pin_reset, pin_dio, pin_busy, pin_rxen, pin_txen);
    ...
    update_display();
      ...
      bt_init();
      bt_init_ran = true;
      #if (BOARD_MODEL == BOARD_CARDPUTER_ADV || BOARD_MODEL == BOARD_TPAGER) && HAS_BLE == true
...
void sleep_now() {
    ...
    #if PLATFORM == PLATFORM_ESP32
      #if BOARD_MODEL == BOARD_T3S3 || BOARD_MODEL == BOARD_XIAO_S3 || BOARD_MODEL == BOARD_CARDPUTER_ADV || BOARD_MODEL == BOARD_TPAGER
        #if HAS_DISPLAY
          display_intensity = 0;
          update_display(true);
        #endif

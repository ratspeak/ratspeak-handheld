...
bool sx126x::preInit() {
  ...
  #if BOARD_MODEL == BOARD_T3S3 || BOARD_MODEL == BOARD_HELTEC32_V3 || BOARD_MODEL == BOARD_HELTEC32_V4 || BOARD_MODEL == BOARD_TDECK || BOARD_MODEL == BOARD_TPAGER || BOARD_MODEL == BOARD_XIAO_S3 || BOARD_MODEL == BOARD_CARDPUTER_ADV
...
int sx126x::begin(long frequency) {
  ...
  standby();
    ...
    // T-Pager module runs the SX1262 in DC-DC regulator mode.
    uint8_t reg_mode = 0x01;
    executeOpcode(OP_REGULATOR_MODE_6X, &reg_mode, 1);
  #endif
...
void sx126x::end() {
  ...
  sleep();
    ...
    LORA_SPI.end();
  #else
    // T-Deck/T-Pager display and LoRa share Arduino SPI; ending it crashes the next UI refresh.
  #endif
  _preinit_done = false;
}

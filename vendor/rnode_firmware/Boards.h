...
#include "Modem.h"
  ...
  #define PRODUCT_TPAGER      0xED // LilyGO T-LoRa-Pager (rsPager dual-boot)
  #define BOARD_TPAGER        0x45
  #define MODEL_ED            0xED // LilyGO T-Pager, 850-950 MHz
  ...
  #define VALIDATE_FIRMWARE true
    ...
    #define PIN_GPS_RX 34
      ...
      // Display on for the rsPager dual-boot build: node ID, waterfall, BLE pairing PIN.
      #define HAS_DISPLAY true
      #define HAS_CONSOLE false
      #define HAS_WIFI true
      #define HAS_BLUETOOTH false
      ...
      #define HAS_INPUT true
      #define HAS_SLEEP true
      #define PIN_WAKEUP GPIO_NUM_7
      #define WAKEUP_LEVEL 0

      // Power is rail-gated by the XL9555 I2C expander (addr 0x20, SDA=3 SCL=2),
      // not a GPIO. See the BOARD_TPAGER expander init in RNode_Firmware.ino.
      const int pin_poweron = -1;
      const int pin_btn_usr1 = 7; // rotary encoder center click
      ...
      const int DISPLAY_MISO = 33;
      const int DISPLAY_MOSI = 34;
      const int DISPLAY_CLK = 35;
      const int DISPLAY_BL_PIN = 42;

      // TCA8418 keyboard + BQ27220 fuel gauge share the main I2C bus
      #define I2C_SDA 3
      #define I2C_SCL 2

      // No discrete RX/TX LEDs on the T-Pager
      #if HAS_NP == false

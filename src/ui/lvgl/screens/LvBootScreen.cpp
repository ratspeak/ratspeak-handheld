...
void LvBootScreen::showError(const char* message) {
    if (_bar) lv_obj_add_flag(_bar, LV_OBJ_FLAG_HIDDEN);
    if (_lblTitle) lv_obj_align(_lblTitle, LV_ALIGN_TOP_MID, 0, 12);
    auto* detail = makeLabel(_screen, message, &lv_font_rsdeck_12,
                             Theme::TEXT_PRIMARY, Theme::CONTENT_W - 32, LV_TEXT_ALIGN_CENTER);
    lv_label_set_long_mode(detail, LV_LABEL_LONG_WRAP);
    lv_obj_align(detail, LV_ALIGN_TOP_MID, 0, 48);
    auto* hint = makeLabel(_screen, "Restart to retry.\nRestore a backup if this continues.",
                           &lv_font_rsdeck_10, Theme::TEXT_SECONDARY,
                           Theme::CONTENT_W - 32, LV_TEXT_ALIGN_CENTER);

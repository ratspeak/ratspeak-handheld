...
constexpr int kLinksW = Theme::SCREEN_W - kLinksX - kBattW - kWifiW - 8;
...
void LvStatusBar::create(lv_obj_t* parent) {
    ...
    // Center: brand/status text. Leave a fixed slot beside the battery for Wi-Fi.
    ...
    lv_obj_set_style_text_font(_wifiIcon, &lv_font_montserrat_12, 0);
    lv_obj_align(_wifiIcon, LV_ALIGN_RIGHT_MID, -kBattW - 8, 0);
    _wifiSlash = lv_line_create(_wifiIcon);
    static const lv_point_t slash[] = {{0, 13}, {15, 0}};
    lv_line_set_points(_wifiSlash, slash, 2);
    lv_obj_set_style_line_width(_wifiSlash, 2, 0);
    lv_obj_set_style_line_rounded(_wifiSlash, true, 0);
    lv_obj_center(_wifiSlash);
...
void LvStatusBar::setBatteryPercent(int pct) {
    ...
    // Suppress single-percent ADC noise: only update if the value changed by 2% or
    // more. A -1 reading (fuel gauge not ready) always propagates as "--%".
...
void LvStatusBar::refreshIndicators() {
    ...
    if (_wifiIcon) {
        if (_wifiEnabled) lv_obj_clear_flag(_wifiIcon, LV_OBJ_FLAG_HIDDEN);
        else lv_obj_add_flag(_wifiIcon, LV_OBJ_FLAG_HIDDEN);
        lv_obj_set_style_text_color(_wifiIcon,
            lv_color_hex(_wifiActive ? Theme::SUCCESS : Theme::WARNING_CLR), 0);
        lv_obj_set_style_line_color(_wifiSlash, lv_color_hex(Theme::WARNING_CLR), 0);
        if (_wifiActive) lv_obj_add_flag(_wifiSlash, LV_OBJ_FLAG_HIDDEN);
        else lv_obj_clear_flag(_wifiSlash, LV_OBJ_FLAG_HIDDEN);
    }
